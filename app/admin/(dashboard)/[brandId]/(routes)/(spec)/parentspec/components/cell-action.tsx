...
import { ParentSpecColumn } from "./columns";
...
export const CellAction: React.FC<CellActionProps> = ({
  ...
  const onConfirm = async () => {
    try {
      setLoading(true);
      const response = await axios.delete(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/api/${params.brandId}/parentspec/${data.id}`);
      if(response.data === 'expired_session'){
        ...
        router.refresh();
        toast.success('Parent Specification deleted.');
      }
    } catch (error) {
      router.refresh()
  ...
  const onCopy = (id: string) => {
    ...
    toast.success('Parent Specification ID copied to clipboard.');
  }
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          >
            ...
            <DropdownMenuItem
              onClick={() => router.push(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/${params.brandId}/parentspec/${data.id}`)}
            >
