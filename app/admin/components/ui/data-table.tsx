...
export function DataTable<TData, TValue>({
  ...
  const indexPreviewNewProductBoolean = columns.findIndex(item => item.header === 'New Product');
  const indexPreviewArchivedBoolean = columns.findIndex(item => item.header === 'Archived');
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell, indexcol) => (
                    ...
                    ) : (indexcol === indexPreviewFeaturedBoolean) ? (
                      /* @ts-ignore */<TableCell key={cell.id}><div style={{ display: 'flex' }}>
                        {data[Number(row.id)]? data[Number(row.id)].isFeatured === true ? <Check size={30} className="text-green-500"/> : <X size={30} className="text-red-500"/> :<FileQuestion size={30}/>}
                      </div>
                      </TableCell>
                    ) : (indexcol === indexPreviewArchivedBoolean) ? (
                      /* @ts-ignore */<TableCell key={cell.id}><div style={{ display: 'flex' }}>
                        {data[Number(row.id)]? data[Number(row.id)].isArchived === true ? <Check size={30} className="text-green-500"/> : <X size={30} className="text-red-500"/> :<FileQuestion size={30}/>}
                      </div>
                      </TableCell>
                    ) : (indexcol === indexPreviewNewProductBoolean) ? (
                      /* @ts-ignore */<TableCell key={cell.id}><div style={{ display: 'flex' }}>
